<script lang="ts">
	...
	function isSelected(date: CalendarDate) {
      ...
      let selectedDate = new Date(selectedDay);
			return (
				selectedDate.getUTCFullYear() === date.year &&
        selectedDate.getUTCMonth() === date.month - 1 && // getUTCMonth() returns a zero-based month, so subtract 1
        selectedDate.getUTCDate() === date.day
			);
		});
    return isSelected
	}
						...
						<tr>
							{#each weekDates as date}
								<td
									...
									class={isSelected(date) ? 'bg-blue-300' : ''}
								>
...
<style>
	...
	[data-melt-calendar-prevbutton] {
		@apply hover:bg-magnum-500/20 rounded-lg p-1 transition-all;
	}
	...
	[data-melt-calendar-cell] {
		@apply hover:bg-magnum-500/20 focus:ring-magnum-400 flex h-6 w-6 cursor-pointer select-none items-center justify-center rounded-lg  p-4 focus:ring;
	}
